...
const rules: RouteRule[] = [
  { kind: "fpa.headcount_plan", weight: 3, any: [/\bfully[- ]loaded\b/i, /\bheadcount\b/i, /\b(cost|price) (of|to) (an? |the )?(new )?(hire|employee|engineer|developer|salesperson|worker)\b/i, /\b(hire|employee) .* cost\b/i, /\bloaded cost\b/i], none: [/\bshould (we|i) hire\b/i, /\bbusiness case\b/i, /\bworth (it|hiring)\b/i], params: (m, e, _asOf) => { const monthly = /\b(per|a|\/)\s?month\b/i.test(m); const netOnly = /\b(net|take[- ]home)\b/i.test(m) && !/\bgross\b/i.test(m); return netOnly ? { grossAnnual: undefined, grossMonthly: undefined } : { grossAnnual: monthly ? undefined : e.amounts[0], grossMonthly: monthly ? e.amounts[0] : undefined, benefitsMonthly: undefined }; } },
  ...
  { kind: "fpa.build_budget", weight: 2.8, all: [/\bbudget\b/i], any: [/\bbuild\b/i, /\bcreate\b/i, /\bprepare\b/i, /\bdraft\b/i, /\bput together\b/i, /\bannual budget\b/i, /\bnext year/i], none: [/\b(department|team|project|engineering|sales|marketing|product|ops|operations)('s)?\s+budget\b/i], params: (_m, e, asOf) => ({ fiscalYear: e.years[0] ?? Number(asOf.slice(0, 4)) + 1 }) },
