  ...
  return (
    <>
      ...
      <Section title="Profile & privacy" id="privacy">
        <Card>
          <p className="text-[13.5px]">{viewer.user.name} · {viewer.user.email}{viewer.person?.title ? ` · ${viewer.person.title}` : ""}</p>
      ...
      <Section title="Appearance" id="appearance">
        <Card>
          ...
          <p className="mt-2 text-[12.5px] text-ink-3">Saved per person. “Match system” follows your device setting.</p>
        </Card>
      </Section>

      <Section title="Assistant & voice" id="assistant">
        <Card>
          <p className="text-[13.5px]" data-testid="assistant-config">
          ...
          {!config.anthropicApiKey && <pre className="mt-2 overflow-x-auto rounded-lg bg-surface-3 p-3 text-[12.5px]">{`ANTHROPIC_API_KEY=sk-ant-...      # server environment only\nFINANCE_DESK_MODEL=claude-opus-5   # optional override`}</pre>}
          <div className="mt-4">
      ...
      <Section title="Integrations" id="integrations">
        <Card>
          <p className="text-[13.5px]">No bank, payroll, card or accounting connections. Data arrives by manual entry, receipts and CSV import.</p>
          <ul className="mt-2 list-disc space-y-1 pl-5 text-[12.5px] text-ink-2">
            ...
            <li>Payroll: copy the withholding lines from a pay stub into Payroll below to make take-home verified.</li>
            <li>Accounting: use “Export my visible data” and share the JSON with your CPA.</li>
          </ul>
      ...
      <Section title="Payroll" id="payroll" description="Gross pay and withholding per person. Federal and state income tax withholding is never assumed.">
      ...
      {viewer.spaces.some((sp) => sp.kind === "company") && (
        <Section title="Company, tax & budget" id="company" description="Every company figure derives from these. Blank means unknown and is never treated as zero.">
          <Card>
      ...
      <Section title="Plan review" id="plan-review" description="What is still unknown, what was superseded, and template records you can archive.">
        <div className="grid gap-3">
          <Card testId="review-queue">
            <p className="label">Open review items</p>
            {items.length === 0 ? <p className="mt-1 text-[13px] text-ink-3">Nothing outstanding.</p> : (
              <ul className="mt-2 divide-y divide-line">
                {items.map((i) => (
                  <li key={i.key} className="py-2 text-[13.5px]">
                    <span className={`chip mr-2 ${i.severity === "attention" ? "chip-warn" : "chip-neutral"}`}>{i.severity}</span>{i.label}
                    <span className="block text-[12px] text-ink-3">{i.blocks} · {i.where}</span>
                  </li>
                ))}
              </ul>
            )}
          ...
          <Card testId="superseded">
            ...
            {viewer.assumptions.superseded.length === 0 ? <p className="mt-1 text-[13px] text-ink-3">None.</p> : (
              <ul className="mt-2 divide-y divide-line">
                {viewer.assumptions.superseded.map((x, idx) => (
                  <li key={`${x.key}-${idx}`} className="py-2 text-[13.5px]">
                    <span className="line-through">{x.label}: {x.previousValue}</span>
                    <span className="block text-[12px] text-ink-3">Archived {x.supersededAt.slice(0, 10)} · {x.reason}</span>
                  </li>
                ))}
              </ul>
            )}
            {history.length > 0 && <p className="mt-2 text-[12px] text-ink-3">{history.length} earlier assumption version{history.length === 1 ? "" : "s"} kept with provenance ({Array.from(new Set(history.map((h) => h.provenance))).join(", ")}).</p>}
          </Card>
          <Card testId="plan-records">
            <p className="label">Goals and budgets on record</p>
            {planRecords.length === 0 ? <p className="mt-1 text-[13px] text-ink-3">None. Targets and allocations live in Money → My money and are set by you, not imposed.</p> : (
              <ul className="mt-2 divide-y divide-line">
                {planRecords.map((r) => (
                  <li key={r.id} className="flex flex-wrap items-center justify-between gap-2 py-2 text-[13.5px]">
                    <span>{r.label}<span className="ml-2 text-[12px] text-ink-3">{r.space.name} · {r.entity} · source {r.source}{r.amount === null ? "" : ` · ${formatCents(r.amount)}/mo`}</span></span>
      ...
      <Section title="Members & permissions" description="Roles apply per space. Personal spaces are private and cannot be shared.">
        <Card className="!p-0">
      ...
      {isOwner && (
        <Section title="Invite links" description="Finance Desk sends nothing. Create a link and share it yourself; it expires in seven days.">
          <Card>
